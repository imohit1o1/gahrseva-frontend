...
export function CustomerBookingList({ bookings, isLoading }: CustomerBookingListProps) {
  return (
    <div className="grid gap-4 min-h-[400px]">
      {isLoading ? (
        ...
        bookings.map((booking) => (
          <div
            key={booking._id}
            className="group relative bg-background border border-border/60 rounded-3xl p-6 transition-all hover:shadow-xl hover:shadow-primary/5"
          >
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                ...
                <div className="space-y-1">
                  ...
                  <div className="flex items-center gap-4 text-xs text-muted-foreground">
                    ...
                    <div className="flex items-center gap-1">
                      <Clock size={14} />
                      <span>
                        {new Date(booking.schedule_at).toLocaleTimeString([], {
                          hour: '2-digit',
                          minute: '2-digit',
                        })}
                      </span>
              ...
              <div className="flex flex-col sm:flex-row items-start sm:items-center gap-6">
                <div className="flex flex-col">
                  ...
                  <div className="flex items-center gap-1 text-xs font-medium">
                    ...
                    <span className="line-clamp-1 max-w-[200px]">
                      {booking.address
                        ? `${booking.address.area ?? ''}${booking.address.area && (booking.address.city || booking.address.pincode) ? ', ' : ''}${booking.address.city ?? ''}${booking.address.pincode ? ` - ${booking.address.pincode}` : ''}`
                ...
                <div className="flex gap-2 w-full sm:w-auto">
                  <button
                    type="button"
                    className="flex-1 sm:flex-none px-4 py-2 rounded-xl border border-border/60 text-xs font-bold hover:bg-accent transition-colors"
                  >
                    Details
                  </button>
                </div>
